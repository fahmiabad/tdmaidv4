...
three workflow modes, time-based inputs, and target-level selection.
"""
import os
import math
...
# --- 1. SECRET ---
# Set your OpenAI API key for deployment on Streamlit Cloud/GitHub
...
target_level = st.sidebar.selectbox(
    "Select target:",
    [
        "Empirical (AUC24 400-600 mg·h/L; trough 10-15 mg/L)",
        "Definitive (AUC24 600-800 mg·h/L; trough 15-20 mg/L)"
    ]
)
# Determine numeric trough target for dosing
...
age = st.sidebar.number_input("Age (yr)", min_value=0, value=65)
wt = st.sidebar.number_input("Weight (kg)", min_value=0.0, value=70.0)
scr = st.sidebar.number_input("Scr (mg/dL)", min_value=0.1, value=1.0)
fem = st.sidebar.checkbox("Female")

# Helper to build report

def build_report(lines):
    hdr = [
        f"Patient ID: {pid}", f"Ward: {ward}", f"Date: {datetime.now():%Y-%m-%d %H:%M:%S}",
        f"Age: {age}, Wt: {wt} kg, Female: {fem}", f"Scr: {scr} mg/dL", f"Target: {target_level}"
    ]
    return "\n".join(hdr + lines)

# Mode: Initial Dose\ nif mode == "Initial Dose":
    st.header("Initial Loading Dose Suggestion")
    if st.button("Calculate Loading Dose"):
        ...
        st.download_button("Download TXT", data=rpt, file_name=f"{pid}_initial.txt")

# Mode: Trough-Only
elif mode == "Trough-Only":
    st.header("Trough-Only Analysis")
    dose_time = st.sidebar.time_input("Last Dose Time", value=time(8,0), step=900)
    sample_time = st.sidebar.time_input("Trough Sample Time", value=time(20,0), step=900)
    dose_int = st.sidebar.number_input("Dose per Interval (mg)", min_value=0, step=250, value=1000)
    trough = st.sidebar.number_input("Measured Trough (mg/L)", min_value=0.1, value=10.0)
    ...
    if st.button("Run Trough-Only"):
        ...
        new_dose = round_dose(calculate_new_dose_trough(dose_int, vd, trough, interval_h, target_trough))
        auc24 = calculate_auc24_trough(dose_int, vd, trough, interval_h)
        st.write(f"CrCl: {crcl:.1f} mL/min | Vd: {vd:.1f} L")
        st.write(f"Interval: {interval_h:.2f} h | Ke: {ke:.3f} h⁻¹ | t½: {t_half:.1f} h")
        st.write(f"AUC24: {auc24:.1f} mg·h/L | New Dose: {new_dose} mg q{interval_h:.1f}h")
        res = {'mode':'trough','interval':interval_h,'ke':ke,'t_half':t_half,'AUC24':auc24,'new_dose':new_dose,'target':target_level}
        interp = interpret(crcl, res, target_level)
        st.subheader("Interpretation")
        st.write(interp)
        rpt = build_report([
            f"Mode: Trough-Only", f"Dose Time: {dose_time}", f"Sample Time: {sample_time}",
            f"Trough: {trough} mg/L", f"New Dose: {new_dose} mg q{interval_h:.1f}h", f"AUC24: {auc24:.1f}", f"Interp: {interp}"
        ])
        st.download_button("Download TXT", data=rpt, file_name=f"{pid}_trough.txt")

# Mode: Peak & Trough\ nelse:
    st.header("Peak & Trough Analysis")
    start = st.sidebar.time_input("Infusion Start Time", value=time(8,0), step=900)
    end = st.sidebar.time_input("Infusion End Time", value=time(9,0), step=900)
    peak = st.sidebar.time_input("Peak Sample Time", value=time(10,0), step=900)
    trough_time = st.sidebar.time_input("Trough Sample Time", value=time(20,0), step=900)
    cmin = st.sidebar.number_input("Trough (Cmin) mg/L", min_value=0.1, value=10.0)
    cpost = st.sidebar.number_input("Post-infusion (Cpost) mg/L", min_value=0.1, value=30.0)
    ...
    if st.button("Run Peak & Trough"):
        crcl = calculate_crcl(age, wt, scr, fem)
        params = calculate_prepost(cmin, cpost, infusion_h, peak_delay_h, interval_h)
        st.write(f"CrCl: {crcl:.1f} mL/min | Vd: {calculate_vd(wt, age):.1f} L")
        st.write(f"Infusion: {infusion_h:.2f} h | Peak Delay: {peak_delay_h:.2f} h | Interval: {interval_h:.2f} h")
        st.write(f"Ke: {params['ke']:.3f} h⁻¹ | t½: {params['t_half']:.1f} h")
        st.write(f"Cmax: {params['Cmax']:.1f} mg/L | Cmin: {params['Cmin']:.1f} mg/L")
        ...
        st.write(interp)
        rpt = build_report([
            f"Mode: Peak & Trough", f"Start: {start}", f"End: {end}", f"Peak: {peak}", f"Trough: {trough_time}",
            f"Cmin: {cmin}", f"Cpost: {cpost}", f"AUC24: {params['AUC24']:.1f}", f"Interp: {interp}"
        ])
        st.download_button("Download TXT", data=rpt, file_name=f"{pid}_prepost.txt")
