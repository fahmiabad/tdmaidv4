...
def calculate_individual_vd(dose: float, weight: float, cmax: float, ke: float, interval_h: float) -> float:
    ...
    try:
        ...
        vd_per_kg = dose / (weight * cmax * term_exp)

        # Convert to absolute Vd in L
        vd_absolute = vd_per_kg * weight

        logging.info(f"Calculated individual Vd: {vd_per_kg:.2f} L/kg ({vd_absolute:.1f} L)")
        return vd_absolute
...
def calculate_pk_params_peak_trough(
    ...
    if time_between_samples_h <= 0 or c_peak_measured <= 0 or c_trough <= 0 or c_peak_measured <= c_trough:
        ...
        return None
    try:
        # Calculate Ke (matches image formula a)
        ...
        if ke <= 0:
             ...
             return None
        # Calculate t1/2 (matches image formula b)
        half_life_h = math.log(2) / ke
        # Calculate Cmax extrapolated (matches image formula c)
        c_max_extrapolated = c_peak_measured * math.exp(ke * time_from_infusion_end_to_peak_draw_h)
        # Use measured trough as actual Cmin
        c_min_actual = c_trough

        # Calculate individual Vd using the image formula (e)
        vd_ind = calculate_individual_vd(dose, weight, c_max_extrapolated, ke, current_interval_h)
        if vd_ind <= 0:
            logging.warning("Individual Vd calculation failed. Cannot proceed.")
            return None

        # AUC calculation using linear-log/trapezoidal method (standard approach)
        auc_inf = infusion_duration_h * (c_min_actual + c_max_extrapolated) / 2
        auc_elim = (c_max_extrapolated - c_min_actual) / ke
        auc_interval_total = auc_inf + auc_elim
        ...
        logging.info(f"Calculated PK Params (peak/trough): Ke={ke:.4f}, t1/2={half_life_h:.1f}, Cmax_extrap={c_max_extrapolated:.1f}, Cmin_actual={c_min_actual:.1f}, Vd_ind={vd_ind:.1f} L, AUC24={auc24:.1f}, ExpCmax={expected_cmax:.1f}, ExpCmin={expected_cmin:.1f}")
        return {
            'ke': ke,
            't_half': half_life_h,
            'Cmax_extrapolated': c_max_extrapolated,
            'Cmin_actual': c_min_actual,
            'Vd_individual': vd_ind, # Vd calculated from formula (e)
            'AUC24': auc24,
...
# --- 8. LLM INTERPRETATION (Refined Prompt) ---
def interpret(crcl: float, pk_results: dict, target_level_desc: str, clinical_notes: str) -> str:
    ...
    half_life_info = f"Calculated half-life is {pk_results.get('t_half', 'N/A'):.1f}h." if 't_half' in pk_results else ""
...
Task: Provide a concise, structured interpretation and recommendation, considering all provided information. Structure your response clearly under the following headings: Assessment, Recommendation, Rationale, Follow-up.

1.  **Assessment:**
    * Are the current levels/AUC within the specified target range ({target_level_desc}) for the current interval ({current_interval_info})? State the target clearly.
    * Is the patient clearing the drug as expected based on CrCl? {time_context}
    * Is the current dosing interval appropriate given the calculated half-life ({half_life_info})?
    * Does the clinical picture (from notes) align with the levels? (e.g., therapeutic levels but ongoing fever?)
    * If available, compare measured levels to the 'Expected_Cmax'/'Expected_Cmin' calculated for the current regimen.

2.  **Recommendation:**
    * Clearly state whether a dose/interval change is needed.
    * If adjustment is needed:
        * Suggest a specific dose (rounded to nearest 250mg) AND a standard clinical interval (e.g., q8h, q12h, q24h).
...
3.  **Rationale:**
    ...
    * Link the decision directly to the PK results (Ke, Vd, AUC, levels vs. target), CrCl, half-life vs. interval, and relevant clinical notes.
...
Use the provided guideline knowledge. Be specific and clinically oriented. Ensure the response is complete.
"""
        ...
        if calc_button:
             if dose_int_current <= 0 or trough_measured <= 0 or time_since_last_dose_h <= 0 or wt <= 0 or age <= 0 or scr_mgdl <= 0 or current_interval_h <= 0:
                ...
                with st.spinner("Analyzing Trough Level..."):
                    ...
                    vd_calc = calculate_population_vd(wt, age)
                    ...
                    if ke_calc > 0 and vd_calc > 0:
                        report_lines = [
                            ...
                            f"Est. Population Vd: {vd_calc:.1f} L", # Clarified Vd type
            ...
            if not valid_times:
                ...
                with st.spinner("Analyzing Peak & Trough Levels..."):
                    ...
                    if pk_params:
                        ...
                        # Get individual Vd calculated using formula (e)
                        vd_ind = pk_params['Vd_individual']
                        # Get expected levels calculated using formulas (f, g)
                        ...
                        col1.metric(label="Individual Vd", value=f"{vd_ind:.1f} L" if vd_ind > 0 else "N/A", help="Calculated using Dose / [BW × Cmax × (1−e−KeT)]")
                        ...
                        # --- Suggested Dose Calculation using Individual Vd from formula (e) ---
                        new_dose_suggestion = "N/A"
                        if ke_ind > 0 and vd_ind > 0 and current_interval_h_pt > 0:
                             cl_ind = ke_ind * vd_ind # Use individual CL based on formula (e) Vd
                    ...
                    if pk_params:
                        report_lines = [
                            ...
                            f"Ind. Vd (Formula e): {vd_ind:.1f} L" if vd_ind > 0 else "N/A", # Specify Vd method
                            f"Ind. Ke: {ke_ind:.4f} h⁻¹",
                            f"Ind. t½: {thalf_ind:.1f} h",
                            f"Est. Cmax (End of Infusion): {cmax_ind:.1f} mg/L",
                            f"Expected Cmax (Formula f): {exp_cmax_ind:.1f} mg/L" if exp_cmax_ind > 0 else "N/A", # Add expected
                            f"Expected Cmin (Formula g): {exp_cmin_ind:.1f} mg/L" if exp_cmin_ind > 0 else "N/A", # Add expected
